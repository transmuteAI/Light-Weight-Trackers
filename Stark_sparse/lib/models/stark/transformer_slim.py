...
from .slim_layers import MultiHeadAttention,MultiHeadSparseAttention,Mlp,SparseMlp,ModuleInjection
from .slim_base_model import BaseModel
...
class Transformer(BaseModel,nn.Module):

    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 num_decoder_layers=6, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False,
                 return_intermediate_dec=False, divide_norm=False,head_search=False, uniform_search=False):
        super().__init__()

       
        
#         encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
#                                                 dropout, activation, normalize_before, divide_norm=divide_norm,head_search = head_search,uniform_search = uniform_search)
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        if num_encoder_layers == 0:
            self.encoder = None
        else:
            self.encoder = TransformerEncoder(d_model, nhead, dim_feedforward,dropout,
                                              activation, normalize_before, divide_norm,
                                              head_search,uniform_search,num_encoder_layers,encoder_norm)
...
#                                                 dropout, activation, normalize_before, divide_norm=divide_norm,head_search = head_search,uniform_search = uniform_search)
        ...
        if num_decoder_layers == 0:
            ...
            self.decoder = TransformerDecoder(d_model, nhead, dim_feedforward,
                                              dropout, activation, normalize_before, divide_norm,head_search,
                                              uniform_search, num_decoder_layers, decoder_norm,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,d_model,nhead,dim_feedforward,dropout,activation,normalize_before,divide_norm,head_search,uniform_search,num_layers, norm=None):
        super().__init__()
#         self.layers = _get_clones(encoder_layer, num_layers)
        self.layers = nn.ModuleList([TransformerEncoderLayer(d_model, nhead,dim_feedforward,
                                                             dropout, activation,normalize_before, 
                                                             divide_norm,head_search,uniform_search) for i in range(num_layers)])
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,d_model,nhead,dim_feedforward,dropout,activation,normalize_before,divide_norm,head_search,uniform_search,num_layers,
                 norm=None, return_intermediate=False):
        super().__init__()
#         self.layers = _get_clones(decoder_layer, num_layers)
        self.layers = nn.ModuleList([TransformerDecoderLayer(d_model, nhead, dim_feedforward,
                                                             dropout, activation, normalize_before, 
                                                             divide_norm,head_search,uniform_search) for i in range(num_layers)])
        self.num_layers = num_layers
        self.norm = norm
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, divide_norm=False,head_search=False, uniform_search=False):
        super().__init__()
#         self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        
        self.self_attn = MultiHeadAttention(d_model,nhead,qkv_bias=True,attn_drop=dropout)
        self.self_attn = ModuleInjection.make_searchable_attn(self.self_attn, head_search, uniform_search)
        # Implementation of Feedforward model
#         self.linear1 = nn.Linear(d_model, dim_feedforward)
#         self.dropout = nn.Dropout(dropout)
#         self.linear2 = nn.Linear(dim_feedforward, d_model)

        self.mlp = Mlp(in_features = d_model,hidden_features = dim_feedforward,out_features = d_model,act_layer=activation, drop=dropout)
...
#                               key_padding_mask=src_key_padding_mask)[0]
        src2 = self.self_attn(q, k, src, attn_mask = src_key_padding_mask)
#         print('out',src2.shape)
...
#         print(self.self_attn)
        ...
        self.multihead_attn =  MultiHeadAttention(d_model,nhead,qkv_bias=True,attn_drop=dropout)
        self.multihead_attn = ModuleInjection.make_searchable_attn(self.multihead_attn, head_search, uniform_search)
        # Implementation of Feedforward model
#         self.linear1 = nn.Linear(d_model, dim_feedforward)
#         self.dropout = nn.Dropout(dropout)
#         self.linear2 = nn.Linear(dim_feedforward, d_model)
...
#         self.activation = _get_activation_fn(activation)
    ...
    def forward_post(self, tgt, memory,
        ...
        if self.divide_norm:
            q = q / torch.norm(q, dim=-1, keepdim=True) * self.scale_factor
            k = k / torch.norm(k, dim=-1, keepdim=True)
#         print('decoder_input_dim',q.shape)
        tgt2 = self.self_attn(q, k, tgt, attn_mask = tgt_key_padding_mask)
...
#         print('decoder_input_dim_multi_attn',queries.shape)
        tgt2 = self.multihead_attn(queries, keys, memory, attn_mask=memory_key_padding_mask)
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt2 = self.self_attn(q, k, value=tgt2, attn_mask = tgt_key_padding_mask)
        tgt = tgt + self.dropout1(tgt2)
        tgt2 = self.norm2(tgt)
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_key_padding_mask)
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
        tgt2 = self.mlp(tgt2)
        tgt = tgt + self.dropout3(tgt2)
...
def build_sparse_transformer(cfg):
    return Transformer(
        ...
        head_search=cfg.MODEL.HEAD_SEARCH,
        uniform_search=cfg.MODEL.UNIFORM_SEARCH
    )
