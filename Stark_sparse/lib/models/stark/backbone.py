...
from lib.models.stark import resnet_child as resnet_module_child
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
                 ...
                 freeze_bn: bool,
                 ckpt_path=None):
        if "resnet" in name and "child" not in name:
            norm_layer = FrozenBatchNorm2d if freeze_bn else nn.BatchNorm2d
#             norm_layer = FrozenBatchNorm2d if freeze_bn else nn.SyncBatchNorm
#             norm_layer = None
            ...
            n_cfg = torch.load(ckpt_path, map_location='cpu')['cfg']
            
            backbone = getattr(resnet_module_child, name)(
                cfg = n_cfg , replace_stride_with_dilation=[False, dilation, False],
                pretrained=False, norm_layer=norm_layer, last_layer='layer3')
            ...
            try:
                ...
                print("Warning: Pretrained Resnet child weights are not loaded")
...
def build_backbone(cfg):
    ...
    elif cfg.MODEL.BACKBONE.TYPE == "resnet_50_child":
        try:
            ckpt_path = os.path.join(cfg.ckpt_dir, "resnet.pth")
            print(ckpt_path)
        except:
            ckpt_path = None
            print("ckpt_not_defined")
    else:
        ckpt_path = None
        
    if cfg.MODEL.BACKBONE.TYPE == "resnet_50_child":
