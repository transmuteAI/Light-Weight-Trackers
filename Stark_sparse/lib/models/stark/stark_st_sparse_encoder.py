...
from .transformer_slim import build_sparse_transformer
from .transformer_slim_encoder import build_sparse_transformer_encoder
from .head import build_box_head, MLP
from lib.models.stark.stark_s import STARKS
from .slim_layers import ModuleInjection
...
def build_starkst_sparse_encoder(cfg):
    backbone = build_backbone(cfg)  # backbone and positional encoding are built together
    ModuleInjection.searchable_modules = []
    transformer = build_sparse_transformer_encoder(cfg)
    transformer.searchable_modules = ModuleInjection.searchable_modules
    print('searchable_module_coming',len(ModuleInjection.searchable_modules),' ','searchable_module_actual',12)
