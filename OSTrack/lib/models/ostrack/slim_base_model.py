...
#         zetas_patch = sorted(zetas_patch)
        ...
        l_mlp = []
        mlp_array = np.array(zetas_mlp)
        for i in range(mlp_array.shape[0]):
            mlp_array1 = np.sort(mlp_array[i])
            threshold_mlp = mlp_array1[int((1.-budget_mlp)*len(mlp_array1))]
            l_mlp.append(threshold_mlp)
    
        l_attn = []
        attn_array = np.array(zetas_attn)
        for i in range(attn_array.shape[0]):
            attn_array1 = np.sort(attn_array[i])
            threshold_attn = attn_array1[int((1.-budget_attn)*len(attn_array1))]
            l_attn.append(threshold_attn)

#         threshold_attn = zetas_attn[int((1.-budget_attn)*len(zetas_attn))]
#         threshold_mlp = zetas_mlp[int((1.-budget_mlp)*len(zetas_mlp))]
        threshold_patch = zetas_patch[int((1.-budget_patch)*len(zetas_patch))]
#         return threshold_attn, threshold_mlp, threshold_patch
        return l_attn, l_mlp, threshold_patch



    def calculate_search_threshold_min(self, budget_attn, budget_mlp, budget_patch):
        zetas_attn, zetas_mlp, zetas_patch = self.give_zetas()
        
        zetas_attn_global = [z for k in zetas_attn for z in k ]
        zetas_mlp_global = [z for k in zetas_mlp for z in k ]
        zetas_patch = [z for k in zetas_patch for z in k ]
        
        zetas_attn_global = sorted(zetas_attn_global)
        zetas_mlp_global = sorted(zetas_mlp_global)
        zetas_patch = sorted(zetas_patch)
        threshold_attn_global = zetas_attn_global[int((1.-budget_attn)*len(zetas_attn_global))]
        threshold_mlp_global = zetas_mlp_global[int((1.-budget_mlp)*len(zetas_mlp_global))]
        threshold_patch = zetas_patch[int((1.-budget_patch)*len(zetas_patch))]
        
        
        min_budget_mlp = 0.002
        min_budget_attn = 0.002
        print('min_budget = ',min_budget_mlp)
        
        l_mlp = []
        mlp_array = np.array(zetas_mlp)
        for i in range(mlp_array.shape[0]):
            mlp_array1 = np.sort(mlp_array[i])
            threshold_mlp = mlp_array1[int((1.-min_budget_mlp)*len(mlp_array1))]
            l_mlp.append(min(threshold_mlp, threshold_mlp_global))
    
        l_attn = []
        attn_array = np.array(zetas_attn)
        for i in range(attn_array.shape[0]):
            attn_array1 = np.sort(attn_array[i])
            threshold_attn = attn_array1[int((1.-min_budget_attn)*len(attn_array1))]
            l_attn.append(min(threshold_attn, threshold_attn_global))
...
#         self.compress_patch(thresh_patch)
    ...
    def compress_minthresh(self, budget_attn, budget_mlp, budget_patch):
        """compress the network to make zeta exactly 1 and 0"""
        if self.searchable_modules == []:
            self.searchable_modules = [m for m in self.modules() if hasattr(m, 'zeta')]
        
        thresh_attn, thresh_mlp, thresh_patch = self.calculate_search_threshold_min(budget_attn, budget_mlp, budget_patch)
