...
def run_training(name, child_train,script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    settings.child_train = child_train
    settings.name = name
    settings.project_path = 'train/{}/{}/{}'.format(script_name, config_name, name)
...
def main():
    ...
    parser.add_argument('--name', type=str, required=True, help='checkpoint save name.')
    parser.add_argument('--child_train', type=int, choices=[0, 1], default=0)
    ...
    run_training(args.name, args.child_train, args.script, args.config, cudnn_benchmark=args.cudnn_benchmark,
