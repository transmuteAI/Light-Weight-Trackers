...
#         self.base_criterion = base_criterion
    ...
    def forward(self, inputs, actor):
        ...
        if self.settings.sparsity_train and not self.settings.child.train:
            sparsity_loss_attn, sparsity_loss_mlp, sparsity_loss_patch = actor.net.module.backbone.get_sparsity_loss(self.device)
    #         print(sparsity_loss_patch, sparsity_loss_mlp)
            total_loss = base_loss+self.settings.w1*sparsity_loss_attn + self.settings.w2*sparsity_loss_mlp #+ self.settings.w3*sparsity_loss_patch
            return total_loss, base_loss, sparsity_loss_attn, sparsity_loss_mlp, sparsity_loss_patch , stats
        else:
...
class LTRTrainer(BaseTrainer):
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None, use_amp=False):
        ...
        self.sparsity_loss_attn = 0
        self.sparsity_loss_mlp = 0
        ...
        if settings.local_rank in [-1, 0]:
            ...
            if settings.use_wandb:
                assert len(settings.wandb_key)==40
...
#                 self.wandb_writer = WandbWriter(settings.project_path[6:], {}, tensorboard_writer_dir, cur_train_samples, interval)
                if settings.child_train:
                    ...
                    run = wandb.init(project="Pruning_in_Stark",
                         name=settings.name,
                         group='ostrack_ce_child',
                         entity="pruning_in_tracking",
                         resume="allow"
                         )
                elif settings.sparsity_train:
                    
                    wandb.login(key=settings.wandb_key)
                    run = wandb.init(project="Pruning_in_Stark",
                             name=settings.name,
                             group='ostrack_ce_sparse',
                             entity="pruning_in_tracking",
                             resume="allow"
                             )
                else:
...
#                 break
            ...
            if not self.settings.sparsity_train:
                if not self.use_amp:
            ...
            if self.settings.sparsity_train and not self.settings.child_train:
                tk1.set_postfix({'epoch': self.epoch,'s_att':self.settings.w1*self.sparsity_loss_attn.item(),
                                 's_mlp':self.settings.w2*self.sparsity_loss_mlp.item(), 'naive_loss' : loss1.item(),
                                 'total_loss':loss.item()})
            else:
                tk1.set_postfix({'epoch': self.epoch, 'total_loss':loss.item()})
    ...
    def train_epoch(self):
        ...
        if self.settings.local_rank in [-1, 0]:
            ...
            train_loss_location = self.stats['train']["Loss/location"].avg
            ...
            valid_loss_location = self.stats['val']["Loss/location"].avg
            valid_iou = self.stats['val']['IoU'].avg
            
#             if settings.child_train:
#                 wandb.log({
#                     'w1' : self.settings.w1, 'w2' : self.settings.w2, 'attn_loss' : self.sparsity_loss_attn, 'mlp_loss' : self.sparsity_loss_mlp,
...
#                    'valid_loss_clf':valid_loss_location,'valid_iou':valid_iou, 'budget'
#                   })
#             else:
