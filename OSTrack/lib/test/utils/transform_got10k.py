...
def transform_got10k(name, tracker_name, cfg_name):
    env = env_settings()
    result_dir = env.results_path
    src_dir = os.path.join(result_dir, "%s/%s/%s/got10k/" % (tracker_name, cfg_name, name))
    dest_dir = os.path.join(result_dir, "%s/%s/%s/got10k_submit/" % (tracker_name, cfg_name, name))
...
if __name__ == "__main__":
    ...
    transform_got10k(args.name, args.tracker_name, args.cfg_name)
