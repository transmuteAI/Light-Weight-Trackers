...
def get_plot_draw_styles():
    plot_draw_style = [{'color': (1.0, 0.0, 0.0), 'line_style': '-'},
                       ...
                       {'color': (0.7, 0.1, 0.2), 'line_style': '-'},
                       {'color': (0.1, 0.7, 0.4), 'line_style': '-'},
                       {'color': (0.9, 0.3, 0.6), 'line_style': '-'},
                       {'color': (0.2, 0.5, 1.0), 'line_style': '-'},
                       {'color': (0.5, 0.0, 0.0), 'line_style': '-'},
                       {'color': (0.0, 0.0, 0.5), 'line_style': '-'},
...
#         disp_name = tracker['disp_name']
    ...
    disp_name = '{}'.format(str(tracker['disp_name']))
#     disp_name = tracker['disp_name']
#     if tracker['run_id'] is None:
#         disp_name = '{}_{}'.format(tracker['name'], tracker['param'])
#     else:
...
#     plt.rcParams['text.usetex']=True
    ...
    # Plot settings
    font_size = plot_opts.get('font_size', 27)
    font_size_axis = plot_opts.get('font_size_axis', 27)
    line_width = plot_opts.get('line_width', 2)
    font_size_legend = plot_opts.get('font_size_legend', 18)
...
#         plotted_lines.append(line[0])
        ...
        if ("OSTrack" in disp_name):
            color = (0.0, 0.0, 1.0)
            if ("10" in disp_name):
                ls = ':'
            elif  ("25" in disp_name):
                ls = "-."
            elif  ("50" in disp_name):
                ls = "--"
            else :
                ls = "-"
        if ("Dimp" in disp_name):
            color = (0.0, 1.0, 0.0)
            if ("10" in disp_name):
                ls = ':'
            elif ("25" in disp_name):
...
#         legend_text.append('{} [{:.1f}]'.format(disp_name, scores[id_sort]))
        legend_text.append('{:.1f}'.format(scores[id_sort]))
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    # Load pre-computed results
    result_plot_path = os.path.join(settings.result_plot_path, report_name)
#     print(trackers[0].run_id)

#     for trackers in tracker:
