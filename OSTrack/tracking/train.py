...
def parse_args():
    ...
    # for train
    parser.add_argument('--name', type=str, help='checkpoint save file name')
    parser.add_argument('--child_train', type=int, choices=[0, 1], default=0, help='keep it 1 to train child')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --name %s --child_train %d --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s --distill %d --script_teacher %s --config_teacher %s --use_wandb %d"\
                    % (args.name, args.child_train, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv,
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                    " --name %s --child_train %d --script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
                    % (args.nproc_per_node, random.randint(10000, 50000), args.name, args.child_train, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb,
